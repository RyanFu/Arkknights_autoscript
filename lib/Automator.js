let _ra_instance = undefined;

function AutomationWithRoot() {
  if (!(files.exists("/sbin/su") || 
...
AutomationWithRoot.prototype = {
  ...
  gesture: function (argus) {
    if (_ra_instance === undefined) _ra_instance = new RootAutomator();
    let ra = _ra_instance;
...
function Automator() {
  const _automator =
    device.sdkInt < 24 ? new AutomationWithRoot() : new AutomationWithoutRoot();
  return {
    click: function (x, y) {
