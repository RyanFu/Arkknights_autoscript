...
let _a11y = (() => {
  ...
  let _aj_service =
    _aj_package + "/com.stardust.autojs" + ".core.accessibility.AccessibilityService";
    ...
    enable() {
      try {
        ...
        if (_srvs) {
          ...
          if (!_waitForAction(() => _this.state(srvs), 2e3)) {
    ...
    disable() {
      try {
        ...
        if (_srvs) {
          ...
          if (!_waitForAction(() => !_contains(), 2e3)) {
            throw Error("Result Exception");
          }
        }
        return true;
...
function _waitForAction(func, timeout, interval) {
...
function Inspector() {
  this.id = _randomString(8); // 测试单例模式

  this.checkA11yService = function (force) {
    if (_a11y.state()) return true;
...
_ispt_instance = _ispt_instance ? _ispt_instance : new Inspector();

module.exports = _ispt_instance;
